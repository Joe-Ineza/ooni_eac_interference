...
def main() -> None:
    ...
    countries = sorted(rq2_esc["country_name"].dropna().unique().tolist())
    if not countries:
        st.error("No countries found in escalation data.")
        st.stop()

    st.sidebar.header("Dashboard Filters")
    selected_country = st.sidebar.selectbox("Country", countries)

    esc_country = rq2_esc[rq2_esc["country_name"] == selected_country].copy()
    events = sorted(esc_country["event_label"].dropna().unique().tolist())
    if not events:
        st.error("No events found for the selected country.")
        st.stop()
